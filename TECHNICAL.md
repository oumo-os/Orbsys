...
### Periodic aSTF (p-aSTF) — Circle health review

Scheduled per Circle on a staggered cycle anchored to founding date.
Event-triggered on anomaly flags or jSTF pre-referral accumulation.

**Two-layer rubric:**

*Circle rubric (all reviewers, 30 pts)* — based on publicly observable data only:
Activity(6) + Competence Fit(7) + Discipline(6) + Cohesion(5) + Delivery(6)

*Member rubric (assigned reviewers only, 35 pts + 2 risk flags)* — per assigned member:
Effectiveness(5) + Stewardship(7) + Participation(5) + Investment(8) + Productivity(6) + Role Fit(4)
Risk flags (separate, not additive): Replaceability(0–5) · Indispensable(0–5)

**Bipartite assignment:** Each reviewer is assigned 1–3 Circle members; each member receives
2–3 independent reviewers. Reviewers see only their assigned members, not the full Circle.

**Unit of assessment is the individual member.** Circle health score is aggregated from
member-level scores. A Circle with one outlier member is distinguishable from a uniformly
mediocre Circle at the same mean.

**Health tier:** Healthy / Watch / Concern — filed by each reviewer, majority-voted by
Integrity Engine. All scores and tiers are published to the open ledger. Reviewer
attribution is permanently sealed.

**Reviewer availability:** Members declare standing availability for review work (separate
from Curiosity). The Inferential Engine builds review pools from available members on demand.
