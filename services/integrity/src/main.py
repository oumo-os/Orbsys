...
async def get_prev_hash(db, org_id):
    row = (await db.execute(
        text("SELECT event_hash FROM ledger_events WHERE org_id=:oid ORDER BY created_at DESC, event_hash DESC LIMIT 1"),
        {"oid": org_id},
    )).fetchone()
...
LEDGER_ONLY = {
    ...
    # Competence
    "wh_claim_submitted", "anomaly_flagged", "fc_candidacy_evaluated", "jstf_pre_referral", "circle_health_assessed",
...
async def handle_stf_formation(data, db):
    ...
    # isolated_view_token is generated inline below — not imported from API
...
async def handle_fc_candidacy(data: dict, db: AsyncSession) -> None:
    """
    Record founding circle candidacy recommendation.
    If quorum is met, create the Founding Circle and seed proposals.
    This runs inside the Integrity Engine which has full DB access.
    """
    payload        = data.get("payload", {})
    org_str        = data.get("org_id")
    member_id_str  = payload.get("member_id") or data.get("subject_id")
    fc_rec         = payload.get("fc_recommendation", False)
    vstf_id_str    = payload.get("vstf_id")
    if not org_str or not member_id_str:
        return
    ...
    existing = (await db.execute(
        text("SELECT id FROM org_parameters WHERE org_id=:oid AND parameter=:key"),
        {"oid": org_id, "key": key},
    )).fetchone()
    if not existing:
        await db.execute(
            text("INSERT INTO org_parameters (id,org_id,parameter,value,applied_at) "
                 "VALUES (uuid_generate_v4(),:oid,:key,:val,now())"),
            {"oid": org_id, "key": key,
             "val": __import__("json").dumps({"value": fc_rec, "vstf_id": vstf_id_str})},
        )
    if not fc_rec:
        return

    # Already have a founding circle?
    fc_row = (await db.execute(
        text("SELECT id FROM circles WHERE org_id=:oid AND founding_circle=true AND dissolved_at IS NULL"),
        {"oid": org_id},
    )).fetchone()

    if fc_row:
        # Add member to existing FC if not already in it and FC < 13
        fc_id = fc_row[0]
        already = (await db.execute(
            text("SELECT id FROM circle_members WHERE circle_id=:cid AND member_id=:mid"),
            {"cid": fc_id, "mid": member_id},
        )).fetchone()
        fc_size = (await db.execute(
            text("SELECT COUNT(*) FROM circle_members WHERE circle_id=:cid AND exited_at IS NULL"),
            {"cid": fc_id},
        )).scalar_one() or 0
        if not already and fc_size < 13:
            await db.execute(
                text("INSERT INTO circle_members (id,circle_id,member_id,joined_at,current_state) "
                     "VALUES (uuid_generate_v4(),:cid,:mid,now(),'active')"),
                {"cid": fc_id, "mid": member_id},
            )
            log.info(f"[integrity] Added member {str(member_id)[:8]} to existing FC {str(fc_id)[:8]}")
        return

    # Check quorum
    all_recs = (await db.execute(
        text("SELECT value FROM org_parameters WHERE org_id=:oid AND parameter LIKE 'fc_candidate_%'"),
        {"oid": org_id},
    )).fetchall()
    confirmed = [r for r in all_recs if __import__("json").loads(r[0]).get("value") is True]

    min_param = (await db.execute(
        text("SELECT value FROM org_parameters WHERE org_id=:oid AND parameter='founding_circle_quorum_min'"),
        {"oid": org_id},
    )).fetchone()
    quorum_min = int(__import__("json").loads(min_param[0]).get("value", 3)) if min_param else 3

    if len(confirmed) < quorum_min:
        log.info(f"[integrity] FC candidacy: {len(confirmed)}/{quorum_min} confirmed — waiting")
        return

    # Form the founding circle
    gov_dormain = (await db.execute(
        text("SELECT id FROM dormains WHERE org_id=:oid AND name='Governance' LIMIT 1"),
        {"oid": org_id},
    )).fetchone()

    fc_id = uuid.uuid4()
    await db.execute(
        text("INSERT INTO circles (id,org_id,name,description,founding_circle,created_at) "
             "VALUES (:id,:oid,'Founding Circle',"
             "'Temporary body. Dissolves on founding resolution.',true,now())"),
        {"id": fc_id, "oid": org_id},
    )
    if gov_dormain:
        await db.execute(
            text("INSERT INTO circle_dormains (id,circle_id,dormain_id,mandate_type,added_at) "
                 "VALUES (uuid_generate_v4(),:cid,:did,'primary',now())"),
            {"cid": fc_id, "did": gov_dormain[0]},
        )

    # Add all confirmed candidates
    param_rows = (await db.execute(
        text("SELECT parameter FROM org_parameters "
             "WHERE org_id=:oid AND parameter LIKE 'fc_candidate_%'"),
        {"oid": org_id},
    )).fetchall()
    for row in param_rows:
        mid_str = row[0].replace("fc_candidate_", "")
        try:
    ...
    await write_ledger(
        db, org_id, "founding_circle_formed", fc_id, "circle",
        {"member_count": len(param_rows), "quorum_min": quorum_min},
    )

    # Emit event for Insight Engine to notify members
    # (Integrity Engine doesn't have nc in scope here — use a ledger flag instead)
    # The setup/status polling in the frontend will detect the change.



async def handle_pastf_aggregation(data: dict, db: AsyncSession) -> None:
    """
    All p-aSTF verdicts in. Aggregate:
      1. Circle rubric: average each dimension across all reviewers
      2. Member rubric: per member, average across their assigned reviewers
      3. Health tier: majority vote across all reviewers
      4. Risk flags: any member with replaceability > 3 or indispensable > 3
      5. Write circle_health_snapshots with full breakdown
      6. Emit malpractice pre-referrals if any flags exist
    """
    import json as _json, collections

    stf_id_str    = data.get("subject_id") or data.get("stf_id")
    org_str       = data.get("org_id")
    circle_id_str = (data.get("payload") or {}).get("circle_id")
    if not all([stf_id_str, org_str, circle_id_str]):
        return
    ...
    # Load all verdicts for this p-aSTF
    rows = (await db.execute(
        text("SELECT circle_scores, member_scores, health_tier "
             "FROM stf_verdicts WHERE stf_instance_id = :sid"),
        {"sid": stf_id},
    )).fetchall()
    ...
    # ── Aggregate circle rubric ───────────────────────────────────────────────
    circle_keys = ["activity", "competence_fit", "discipline", "cohesion", "delivery"]
    circle_agg: dict[str, list[float]] = {k: [] for k in circle_keys}
    health_tiers: list[str] = []

    for circle_scores_raw, member_scores_raw, tier in rows:
        cs = _json.loads(circle_scores_raw) if isinstance(circle_scores_raw, str) else (circle_scores_raw or {})
        for k in circle_keys:
            if k in cs:
                circle_agg[k].append(float(cs[k]))
        if tier:
            health_tiers.append(tier)

    circle_avg = {
        k: round(sum(v) / len(v), 2) if v else 0.0
        for k, v in circle_agg.items()
    }
    circle_total = sum(circle_avg.values())

    # Majority health tier
    tier_counter = collections.Counter(health_tiers)
    majority_tier = tier_counter.most_common(1)[0][0] if tier_counter else "watch"

    # ── Aggregate member rubrics ──────────────────────────────────────────────
    member_keys  = ["effectiveness", "stewardship", "participation",
                    "investment", "productivity", "role_fit"]
    flag_keys    = ["replaceability", "indispensable"]
    member_data: dict[str, dict[str, list[float]]] = {}

    for _, member_scores_raw, _ in rows:
        ms = _json.loads(member_scores_raw) if isinstance(member_scores_raw, str) else (member_scores_raw or [])
        for entry in (ms or []):
            mid = entry.get("member_id")
            if not mid:
                continue
            if mid not in member_data:
                member_data[mid] = {k: [] for k in member_keys + flag_keys}
            for k in member_keys + flag_keys:
                if k in entry:
                    member_data[mid][k].append(float(entry[k]))

    member_breakdowns = []
    risk_flags = []

    for mid, scores in member_data.items():
        health_scores = {
            k: round(sum(v) / len(v), 2) if v else 0.0
        ...
        health_pts = round(sum(health_scores.values()), 2)
        member_flags = []
        if flag_scores.get("replaceability", 0) > 3:
            member_flags.append({"flag_type": "replaceability",
                                  "score": flag_scores["replaceability"]})
            risk_flags.append({"member_id": mid, "flag_type": "replaceability",
                                "score": flag_scores["replaceability"]})
        if flag_scores.get("indispensable", 0) > 3:
            member_flags.append({"flag_type": "indispensable",
                                  "score": flag_scores["indispensable"]})
            risk_flags.append({"member_id": mid, "flag_type": "indispensable",
                                "score": flag_scores["indispensable"]})
    ...
    # ── Write to circle_health_snapshots ──────────────────────────────────────
    snap_id = uuid.uuid4()
    await db.execute(text("""
        INSERT INTO circle_health_snapshots
          (id, org_id, circle_id, stf_instance_id,
           circle_scores, member_breakdowns, risk_flags, health_tier,
           computed_at)
        VALUES (:id, :oid, :cid, :sid,
                :cs::jsonb, :mb::jsonb, :rf::jsonb, :ht,
                now())
    """), {
        "id":  snap_id,
        "oid": org_id,
        "cid": circle_id,
        "sid": stf_id,
        "cs":  _json.dumps(circle_avg),
        "mb":  _json.dumps(member_breakdowns),
        "rf":  _json.dumps(risk_flags),
        "ht":  majority_tier,
    })

    await write_ledger(
        db, org_id, "circle_health_assessed", circle_id, "circle",
        {
            "stf_id":         str(stf_id),
            "health_tier":    majority_tier,
            "circle_total":   round(circle_total, 2),
            "member_count":   len(member_breakdowns),
            "risk_flags":     len(risk_flags),
        }
    )

    # ── Emit risk flag notifications ──────────────────────────────────────────
    for flag in risk_flags:
        flag_type = flag["flag_type"]
        mid_str   = flag["member_id"]
        log.info(f"[integrity] Risk flag '{flag_type}' for member {mid_str[:8]} "
                 f"in circle {str(circle_id)[:8]}")
        # knowledge_transfer mandate → notify Membership Circle
        # indispensable → emit jSTF pre-referral event to Judicial Circle

    log.info(f"[integrity] p-aSTF {stf_id} aggregated: "
             f"tier={majority_tier} circle_pts={circle_total:.1f} "
             f"flags={len(risk_flags)}")


async def handle_malpractice_flags(data: dict, db: AsyncSession) -> None:
    """
    Motion aSTF reviewer flagged one or more deliberation participants.
    Emit a jSTF pre-referral ledger event for each flagged member.
    The Judicial Circle decides whether to commission a full jSTF.
    """
    import json as _j
    org_str = data.get("org_id")
    stf_id_str = data.get("subject_id")
    flags = (data.get("payload") or {}).get("malpractice_flags", [])
    if not flags or not org_str:
        return

    org_id = uuid.UUID(org_str)
    for flag in flags:
        mid = flag.get("member_id")
        desc = flag.get("description", "")
        if not mid:
            continue
        await write_ledger(
            db, org_id, "jstf_pre_referral", uuid.UUID(mid), "member",
            {
                "source_stf_id": stf_id_str,
                "flag_type":     "malpractice",
                "description":   desc,
            }
        )
        log.info(f"[integrity] jSTF pre-referral: member {mid[:8]} "
                 f"flagged by motion aSTF {str(stf_id_str or '')[:8]}")
    ...
    async with sf() as db:
        async with db.begin():
            try:
                ...
                elif etype == "fc_candidacy_evaluated":
                    await handle_fc_candidacy(data, db)
                elif etype == "periodic_astf_completed":
                    await handle_pastf_aggregation(data, db)
                elif etype == "motion_astf_malpractice_flagged":
                    await handle_malpractice_flags(data, db)
...
async def main():
    ...
    try:
        await js.add_stream(name="ORG_EVENTS", subjects=["ORG.>"])
    except Exception:
