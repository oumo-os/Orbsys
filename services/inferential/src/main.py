...
async def _emit_event(nc: Any, org_id: uuid.UUID, event: dict) -> None:
    """Publish event to JetStream — not fire-and-forget raw publish."""
    if nc and nc.is_connected:
        try:
            ...
            log.error(f"[inferential] failed to emit event {event.get('event_type')}: {e}")
...
async def compute_and_store_feed_scores(
    db: AsyncSession,
    nc: Any,
    org_id: uuid.UUID,
    thread_id: uuid.UUID,
    ...
    """
    Compute feed relevance scores for a thread and emit an event
    for the Integrity Engine to upsert into feed_scores.
    v1.0 stub: score = 1.0 for all active members (uniform relevance).
    v1.1: incorporate dormain match, recency, engagement signals.
    """
    members_result = await db.execute(text("""
        SELECT id FROM members
        WHERE org_id = :oid AND current_state IN ('active', 'probationary')
    """), {"oid": org_id})
    member_ids = [str(r[0]) for r in members_result.fetchall()]

    if not member_ids:
        return

    scores = [
        {"member_id": mid, "relevance_score": 1.0, "score_basis": "uniform_v1"}
        for mid in member_ids
    ]

    await _emit_event(nc, org_id, {
        "event_type": "feed_scores_write_requested",
        "org_id": str(org_id),
        "thread_id": str(thread_id),
        "scores": scores,
    })
...
async def dispatch(data: dict, sf: async_sessionmaker, nc: Any) -> None:
    ...
    async with sf() as db:
        try:
            if etype == "stf_commissioned":
            ...
            elif etype == "periodic_astf_commissioned":
                # Bipartite reviewer assignment for p-aSTF.
                # Each reviewer gets 1-3 Circle members to assess; each
                # member gets 2-3 independent reviewers.
                stf_id_str  = data.get("subject_id")
                circle_id_str = (data.get("payload") or {}).get("circle_id")
                if not stf_id_str or not circle_id_str:
                    return
                stf_id    = uuid.UUID(stf_id_str)
                ...
                # Load Circle members
                circle_members = (await db.execute(text("""
                    SELECT cm.member_id, m.handle
                    FROM circle_members cm
                    JOIN members m ON m.id = cm.member_id
                    WHERE cm.circle_id = :cid AND cm.exited_at IS NULL
                """), {"cid": circle_id})).fetchall()

                if not circle_members:
                    log.warning(f"[inferential] p-aSTF {stf_id}: no circle members found")
                    return

                n_members = len(circle_members)
                min_reviewers_per_member = 2
                max_members_per_reviewer = 3

                # Score reviewer candidates
                scored = await score_candidates(
                    db, org_id, stf_id, "astf",
                    f"Periodic review of circle {circle_id}",
                    None, None, None,
                )
                # Exclude Circle members from reviewing their own Circle
                circle_member_ids = {str(r[0]) for r in circle_members}
                scored = [s for s in scored if str(s["member_id"]) not in circle_member_ids]

                if not scored:
                    log.warning(f"[inferential] p-aSTF {stf_id}: no eligible reviewers")
                    return

                # Minimum reviewers needed
                min_pool = math.ceil(n_members * min_reviewers_per_member / max_members_per_reviewer)
                pool = scored[:max(min_pool, STF_MAX_SIZE)]

                # Bipartite assignment: round-robin with score weighting
                assignments_map: dict[str, list[str]] = {str(r["member_id"]): [] for r in pool}

                pool_ids = [str(r["member_id"]) for r in pool]

                for cm_id, _ in circle_members:
                    cm_str = str(cm_id)
                    reviewers_assigned = 0
                    for r_id in pool_ids:
                        if reviewers_assigned >= min_reviewers_per_member:
                            break
                        if len(assignments_map[r_id]) >= max_members_per_reviewer:
                            continue
                        assignments_map[r_id].append(cm_str)
                        reviewers_assigned += 1

                # Build assignment list and emit stf_formation_requested
                # Integrity Engine will create the actual DB rows
                assignments = [
                    {"member_id": r_id, "slot_type": "standard",
                     "assigned_member_ids": ami}
                    for r_id, ami in assignments_map.items() if ami
                ]
                ...
                if assignments:
                    await _emit_event(nc, org_id, {
                        ...
                        "org_id": str(org_id),
                        "stf_id": str(stf_id),
                        "stf_type": "periodic_astf",
                        "assignments": assignments,
                        "is_blind": True,
                    })

                log.info(
                    f"[inferential] p-aSTF {stf_id}: "
                    f"{len(assignments)} reviewers assigned "
                    f"to {n_members} Circle members"
                )
                ...
                if not all([cred_id_str, dormain_id_str, member_id_str]):
                    return
                # Emit stf_commissioned — Integrity Engine creates the instance
                stf_id = uuid.uuid4()
                mandate = f"Verify W_h credential claim for dormain {dormain_id_str}"
                await _emit_event(nc, org_id, {
                    "event_type": "stf_commissioned",
                    "org_id": str(org_id),
                    "subject_id": str(stf_id),
                    "subject_type": "stf_instance",
                    "payload": {
                        "stf_type": "vstf",
                        "mandate": mandate,
                        "subject_member_id": member_id_str,
                ...
                # W_h verified → evaluate founding circle candidacy during bootstrap
                payload       = data.get("payload", {})
                member_id_str = payload.get("member_id") or data.get("triggered_by_member")
                vstf_id_str   = payload.get("vstf_id")
                if not member_id_str:
                    return
                # Only act if org is still in bootstrap
                org_row = (await db.execute(
                    text("SELECT bootstrapped_at FROM orgs WHERE id = :oid"),
                    {"oid": str(org_id)},
                )).fetchone()
                if not org_row or org_row[0] is not None:
                    return
                # Count verified credentials for this member
                verified_count = (await db.execute(
                    text("SELECT COUNT(*) FROM wh_credentials "
                         "WHERE org_id=:oid AND member_id=:mid AND status='enacted'"),
                    {"oid": str(org_id), "mid": member_id_str},
                )).scalar_one() or 0
                fc_recommendation = int(verified_count) >= 1
                await _emit_event(nc, org_id, {
                    "event_type": "fc_candidacy_evaluated",
                    "org_id": str(org_id),
                    "subject_id": member_id_str,
                    "subject_type": "member",
                    "payload": {
                        "member_id":            member_id_str,
                        "fc_recommendation":    fc_recommendation,
                        "vstf_id":              vstf_id_str,
                        "verified_credentials": int(verified_count),
                    },
                })
                log.info(f"[inferential] FC candidacy member={str(member_id_str)[:8]} recommend={fc_recommendation}")

            elif etype == "motion_filed":
                # Auto-commission a motion aSTF for Gate 1 review
                motion_id_str = data.get("subject_id")
                payload       = data.get("payload", {})
                cell_id_str   = payload.get("cell_id")
                if not motion_id_str:
                    return
                # Emit stf_commissioned — Integrity Engine creates the instance
                stf_id = uuid.uuid4()
                mandate = (
                    "Gate 1 audit — review motion for process soundness, "
                    "evidence quality, ethical alignment, and conflicts of interest."
                )
                await _emit_event(nc, org_id, {
                    "event_type": "stf_commissioned",
                    "org_id": str(org_id),
                    ...
                    "payload": {
                        "stf_type": "astf",
                        "motion_id": motion_id_str,
                        "cell_id": cell_id_str,
                        "mandate": mandate,
                    },
                })
                log.info(f"[inferential] aSTF {stf_id} auto-commissioned for motion {motion_id_str[:8]}")
...
async def main() -> None:
    ...
    async def handle_msg(msg):
        ...
        if etype in ("stf_commissioned", "commons_thread_created",
                     ...
                     "periodic_astf_commissioned", "wh_claim_submitted",
                     "wh_claim_verified", "motion_filed",
                     "motion_gate1_result", "member_application_submitted",
                     "dormain_tag_applied"):
