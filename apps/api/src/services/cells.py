...
class CellsService(BaseService):
    ...
    async def _maybe_close_vote(
        self, motion: Motion, cell_id: uuid.UUID, org_id: uuid.UUID
    ) -> None:
        """
        Tally all votes for this motion. If weighted-yes fraction >= pass_threshold
        across all dormains combined, close the vote and emit MOTION_VOTE_CLOSED.
        ...
        param_row = (await self.db.execute(
            select(OrgParameter).where(
                ...
                OrgParameter.parameter == "pass_threshold_pct",
            )
        )).scalar_one_or_none()
        threshold = float((param_row.value or {}).get("value", 0.5)) if param_row else 0.5

        # Tally
        rows = (await self.db.execute(
            select(
                CellVote.vote,
                sqlfunc.sum(CellVote.weight).label("total_weight"),
            ).where(CellVote.motion_id == motion.id)
            .group_by(CellVote.vote)
        )).all()

        tally: dict[str, float] = {r.vote: float(r.total_weight) for r in rows}
        yea   = tally.get("yea",     0.0)
        total = sum(tally.values())
        if total == 0:
            return

        # Minimum circle members must have voted (quorum: at least 2)
        vote_count = (await self.db.execute(
            select(func.count(CellVote.id)).where(CellVote.motion_id == motion.id)
        )).scalar_one()
        if vote_count < 2:
            return

        if yea / total >= threshold:
            result = "pass"
        elif (total - yea) / total > (1 - threshold):
            result = "fail"
        else:
        ...
        await get_event_bus().emit(
            org_id,
            GovernanceEvent(
                event_type=EventType.MOTION_VOTE_CLOSED,
                subject_id=motion.id,
                subject_type="motion",
                payload={
                    "result": result,
        ...
        log.info(f"[cells] Motion {motion.id} vote closed — {result}")
    ...
    async def get_minutes(
        ...
        row = (await self.db.execute(
            text(
                "SELECT profile FROM cell_composition_profiles "
                "WHERE cell_id = :cid ORDER BY computed_at DESC LIMIT 1"
            ),
            {"cid": cell_id},
        )).fetchone()
        if row:
        ...
        return {
            ...
            "note": "No minutes snapshot yet — Insight Engine updates after each contribution.",
        }
