...
class CommonsService(BaseService):
    ...
    async def get_tag_suggestions(
        ...
        """
        Dormain tags suggested by the Inferential Engine for this thread.
        Stored in the ledger as "dormain_tag_suggested" events.
        Circle members with relevant mandate can accept via PATCH /dormain-tags.
        """
        ...
        rows = (await self.db.execute(text("""
            ...
            FROM ledger_events
            WHERE org_id = :oid
              AND event_type = 'dormain_tag_suggested'
              AND subject_id = :tid
            ORDER BY created_at DESC
            LIMIT 5
        """), {"oid": org_id, "tid": thread_id})).fetchall()

        import json as _j
        return [
            {
                "suggested_at": r.created_at.isoformat(),
                "dormains": (_j.loads(r.payload) if isinstance(r.payload, str) else r.payload or {}).get("dormains", []),
            }
            for r in rows
        ]
