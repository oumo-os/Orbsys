...
@router.get("/threads/{thread_id}/dormain-tag-suggestions")
async def get_tag_suggestions(
    thread_id: UUID, member: ActiveMember, db: DB
):
    """
    Dormain tag suggestions from the Inferential Engine.
    A Circle member with relevant mandate can apply suggestions via
    PATCH /threads/:id/dormain-tags.
    """
    import uuid as _u
    return await CommonsService(db).get_tag_suggestions(
        _u.UUID(str(thread_id)), _u.UUID(member.org_id)
    )
