...
class CreateCellRequest(_BM):
    ...
    access: str = "closed"   # closed (circle-only) | open (all members can read/post)


@router.post("", status_code=201)
async def create_cell(body: CreateCellRequest, member: ActiveMember, db: DB):
    """
    Direct internal cell creation for Circle members.
    No Commons thread or sponsorship required.
    Use for: internal deliberation, document compilation, progress review.
    """
    return await CellsService(db).create_cell(
        org_id=_u.UUID(member.org_id),
        member_id=_u.UUID(member.member_id),
        mandate=body.mandate,
        circle_id=_u.UUID(body.circle_id) if body.circle_id else None,
        access=body.access,
    )
