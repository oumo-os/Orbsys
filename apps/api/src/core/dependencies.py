...
class CurrentAccount:
    """Platform-level identity — not org membership."""
...
async def get_current_account(
    ...
    """
    Decodes a platform token. Used by /accounts/*, /setup discovery routes,
    and /auth/enter-org. Rejects org session tokens (type != "platform").
    """
    try:
        payload = decode_token(creds.credentials)
        if payload.get("type") != "platform":
            raise JWTError("Not a platform token")
        return CurrentAccount(payload["sub"])
    except (JWTError, KeyError):
        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED,
            "Invalid or expired platform token — use POST /auth/login"
        )


# Accepts EITHER a platform token OR an org session token.
# Useful for endpoints like the unified notification inbox that should
# work whether or not the caller is currently "inside" an org.
async def get_current_account_or_member(
    creds: Annotated[HTTPAuthorizationCredentials, Depends(bearer)],
) -> CurrentAccount:
    try:
        payload = decode_token(creds.credentials)
        ...
        if ptype == "platform":
            return CurrentAccount(payload["sub"])
        if ptype == "access" and payload.get("account"):
            return CurrentAccount(payload["account"])
        raise JWTError("No platform identity in token")
    except (JWTError, KeyError):
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "Invalid token")


PlatformAuth = Annotated[CurrentAccount, Depends(get_current_account)]
AnyAuth      = Annotated[CurrentAccount, Depends(get_current_account_or_member)]
